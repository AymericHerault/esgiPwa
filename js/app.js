...
import { fetchTodos, createTodo, deleteTodo, updateTodo } from './api/todo.js';
import { setTodos, setTodo, getTodos, unsetTodo, getTodoToCreate, getTodoToUpdate, getTodoToDelete } from './idb.js';
// Loading config
      ...
      document.addEventListener('delete-todo', async ({ detail }) => {
        if (!document.offline && navigator.onLine === true) {
          ...
          if (result !== false) {
            ...
            // Rerender the template
            return document.dispatchEvent(new CustomEvent('', { detail: todo }));
          }
        }
        // In case of an error
        detail.deleted = 'true';

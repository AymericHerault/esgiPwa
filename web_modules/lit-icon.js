...
class LitIcon extends HTMLElement {
  constructor() {
    super();
    this._icon = ''; // Create a shadow root
  ...
  findIcon() {
    const iconset = document.iconMap[this._iconset];
    if (!iconset) return; // Create a temporary template to find new created icons
  ...
  _cloneIcon(icon) {
    ...
    let svg = document.createElementNS("http://www.w3.org/2000/svg", "svg"); // Add iron-icon class in order to add control over color

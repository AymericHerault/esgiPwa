...
export default function Home(page, data) {
  ...
  const template = ({ data, properties }) => html`
    <section class="h-full">
      <h1 class="mt-4 px-4 text-xl">My awesome todos : </h1>
      <section class="todolist px-4 mb-20">
        <ul>
      ...
      <footer class="h-16 bg-gray-300 fixed bottom-0 inset-x-0">
        <form @submit="${handleForm}" id="addTodo" class="w-full h-full flex justify-between items-center px-4 py-3">
          <input
            ...
            class="flex-1 py-3 px-4 rounded-sm h-full"
            type="text"
            placeholder="Enter a new todo ..."
            name="todo">
          <button
            class="ml-4 rounded-lg text-uppercase bg-heraku h-full text-center px-3 text-white font-bold flex justify-center items-center"
